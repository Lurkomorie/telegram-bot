...
export default function MultiLineChart({ data, title, height = 300, yAxisLabel = 'Value' }) {
  ...
  const chartData = data.map(item => ({
    time: item.timestamp,
    all: item.avg_waiting_time !== null && item.avg_waiting_time !== undefined ? item.avg_waiting_time : undefined,
    premium: item.avg_premium !== null && item.avg_premium !== undefined ? item.avg_premium : undefined,
    free: item.avg_free !== null && item.avg_free !== undefined ? item.avg_free : undefined
  }));

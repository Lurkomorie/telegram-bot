...
export default function PersonasGallery({
  ...
  const handleDelete = async (personaId, personaName, e) => {
    ...
    // Show confirmation
    const confirmMsg = t("gallery.deleteConfirm").replace(
      "{name}",
      personaName,
    );
    ...
    try {
      ...
      const successMsg = t("gallery.deleteSuccess").replace(
        "{name}",
        personaName,
      );
  ...
  const handleCreated = () => {
    ...
    const shouldContinuePolling = () => {
      ...
      const customCharsWithoutAvatar =
        currentPersonas?.filter((p) => p.is_custom && !p.avatar_url) || [];
      ...
      if (Date.now() - createdAt > 120000) {
        console.log(
          "[GALLERY] ⏰ 2 minutes elapsed, stopping scheduled refreshes",
        );
        createdPersonaCheckRef.stopped = true;
        return false;
      }
  ...
  return (
    <>
      ...
      <div className="shop-banner-gallery" onClick={onNavigateToShop}>
        <Heart className="shop-banner-icon" strokeWidth={2} />
          ...
          >
            <div className="create-card-content">
              <h3 className="create-card-title">
                {t("gallery.createCharacter.title")}
              </h3>
              <p className="create-card-subtitle">
                {t("gallery.createCharacter.subtitle")}
              </p>
            </div>
            {!tokens?.char_created && (
              <div className="create-card-footer">
                <span className="create-card-free-text">
                  {t("gallery.createCharacter.free")}
                </span>
              </div>
            )}
          </div>
  ...
  return (
    <div className="persona-card" onClick={onClick}>
      {persona.avatar_url && !imageError ? (
        ...
        <div
          className={`persona-image-placeholder ${isGeneratingAvatar ? "generating" : ""}`}
        >
          {isGeneratingAvatar ? (
            <>
              <span className="spinner-small"></span>
              <span className="generating-text">
                {t("gallery.arrivingSoon")}
              </span>
            </>
          ) : (

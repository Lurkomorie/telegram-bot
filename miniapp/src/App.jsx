...
function App() {
  ...
  useEffect(() => {
    ...
    const randomKey =
      subscriptionTextKeys[
        Math.floor(Math.random() * subscriptionTextKeys.length)
      ];
  ...
  useEffect(() => {
    ...
    const unsubscribe = onLanguageChange((newLanguage) => {
      console.log(
        `🔄 Language changed to ${newLanguage}, refetching personas and histories...`,
      );
  ...
  const getPageTitle = () => {
    ...
    if (currentPage === "checkout" && selectedTier)
      return t("app.header.checkoutTitle", {
    ...
    if (currentPage === "shop") return t("app.header.shop");
  ...
  return (
    <div className={`app ${showBottomNav ? "app-with-nav" : ""}`}>
      ...
      {showAgeVerification && (
        ...
        >
          ...
          >
            ...
            >
              {isVerifyingAge
                ? t("app.ageVerification.verifying")
        ...
        <header className="app-header gallery-header">
          <div className="gallery-header-content">
            <div className="user-profile-section">
              <div
                className={`user-avatar ${!userPhotoUrl ? "skeleton-wave" : ""}`}
              >
                {userPhotoUrl && (
                  <img
                    src={userPhotoUrl}
              ...
              <div className="user-info-column">
                ...
                <div className="user-energy-display">
                  <img
                    src={lightningIcon}
                    alt="energy"
                    className="energy-icon-small"
                  />
                  <span
                    className={`energy-value-small ${showBonusAnimation ? "energy-boost" : ""}`}
                  >
            ...
            <button
              className="referral-bonus-button"
              onClick={() => handleNavigate("referrals")}
            >
              <span className="bonus-icon">👥</span>
              <span className="bonus-text">
                {t("app.dailyBonus.referralBonus")}
              </span>
            </button>
          </div>
            ...
            >
              <div className="button-content">
                ...
                <span className="button-label">
                  {t("app.dailyBonus.friendBonus")}
                </span>
                <span className="gift-button-day">
                  {t("app.dailyBonus.day", { day: dailyBonusDay })}
                </span>
              </div>
              {tokens.can_claim_daily_bonus ? (
                <div className="button-subtitle gift-button-action">
                  {t("app.dailyBonus.clickToClaim")}
                </div>
              ) : (
                <div className="button-subtitle">
                  {formatTimeUntilBonus(tokens.next_bonus_in_seconds)}
                </div>
              )}
            </button>
            <button
      ...
      <main className="app-main">
        {error ? (
          ...
          <ShopPage
            ...
            onPurchase={(result) => {
              setTokens((prev) => ({ ...prev, tokens: result.new_tokens }));
            }}
            onNavigateToTokens={() => handleNavigate("tokens")}
          />
        ) : null}
      </main>
      ...
      {showBonusCalendar && (
        <BonusCalendar
          onClose={() => setShowBonusCalendar(false)}
          onClaim={(result) => {
            setTokens((prev) => ({
              ...prev,
            ...
            setDailyBonusDay(result.streak);
            setShowBonusAnimation(true);
